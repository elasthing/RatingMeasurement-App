...
import { CopperAnalyzePayload, uploadImage, useAnalyzeCopper } from "@/src/api";
...
function defaultSampleId() {
  ...
  return `CU-${d.getFullYear()}-${p(d.getMonth() + 1)}-${p(d.getDate())}-${rnd}`;
}
...
export default function CopperNewTest() {
  ...
  const [product, setProduct] = useState("Diesel Fuel B30");
  ...
  async function runAnalysis() {
    if (!imageUri) {
      toast("Ambil atau unggah foto copper strip dulu.", "error");
      return;
    }
    setBusy(true);
    ...
    try {
      ...
      const payload: CopperAnalyzePayload = {
        image_path: path,
        sample_id: sampleId,
        product,
        batch,
        operator,
        temperature_c: Number(temperature) || 100,
        duration_hours: Number(duration) || 3,
        remark,
      };
      ...
      router.push(`/copper-result/${result.id}`);
    } catch (e: any) {
  ...
  return (
    <View style={styles.screen}>
      <Header title="New Test" subtitle="Copper Strip AI Vision" logo="CU" showBack />
      ...
      >
        <View style={styles.inner}>
          {imageUri ? (
            <View style={styles.previewWrap}>
              ...
              <View style={styles.cropTag}>
                <Crop size={11} color={colors.onBrandSecondary} weight="bold" />
                <Text style={styles.cropTagText}>CROPPED FOR AI</Text>
              </View>
              <View style={styles.previewActions}>
                <Pressable style={styles.smallBtn} onPress={() => rawImage && setShowCrop(true)} testID="copper-recrop">
                  <Crop size={16} color={colors.onSurface} />
                  <Text style={styles.smallBtnText}>Crop</Text>
                </Pressable>
                <Pressable style={styles.smallBtn} onPress={() => pickImage("camera")} testID="copper-retake-camera">
                  <Camera size={16} color={colors.onSurface} />
                  <Text style={styles.smallBtnText}>Retake</Text>
                </Pressable>
                <Pressable style={styles.smallBtn} onPress={() => pickImage("gallery")} testID="copper-change-gallery">
            ...
            <View style={styles.pickBox}>
              <Text style={styles.pickHint}>Ambil atau unggah foto copper strip</Text>
              <View style={styles.pickRow}>
                <Pressable style={styles.pickBtn} onPress={() => pickImage("camera")} testID="copper-pick-camera">
                  <Camera size={26} color={colors.brandSecondary} weight="fill" />
                  <Text style={styles.pickBtnText}>CAMERA</Text>
                </Pressable>
                <Pressable style={styles.pickBtn} onPress={() => pickImage("gallery")} testID="copper-pick-gallery">
                  <ImageSquare size={26} color={colors.brandSecondary} weight="fill" />
          ...
          <Field label="Sample ID" value={sampleId} onChangeText={setSampleId} testID="copper-input-sample-id" />
          <Field label="Product / Fuel" value={product} onChangeText={setProduct} testID="copper-input-product" />
          <Field label="Batch / Lot No." value={batch} onChangeText={setBatch} placeholder="LOT-…" testID="copper-input-batch" />
          <Field label="Operator" value={operator} onChangeText={setOperator} placeholder="Nama" testID="copper-input-operator" />

          <Text style={styles.sectionLabel}>TEST CONDITION</Text>
          <View style={styles.grid}>
            <Field label="Temp (°C)" value={temperature} onChangeText={setTemperature} numeric half testID="copper-input-temp" />
            <Field label="Duration (h)" value={duration} onChangeText={setDuration} numeric half testID="copper-input-duration" />
          </View>
          <Field label="Remark" value={remark} onChangeText={setRemark} placeholder="Opsional" testID="copper-input-remark" />
        ...
        <View style={[styles.footer, { paddingBottom: Math.max(insets.bottom, spacing.md) }]}>
          ...
          >
            {busy ? (
              ...
              <>
                <Sparkle size={18} color={colors.onBrandSecondary} weight="fill" />
...
const useStyles = makeStyles((c) => ({
  ...
  previewWrap: { borderRadius: radius.lg, overflow: "hidden", borderWidth: 1, borderColor: c.brandSecondary },
  ...
  cropTagText: { fontFamily: fonts.monoBold, fontSize: 9, color: c.onBrandSecondary, letterSpacing: 1 },
  ...
  sectionLabel: { fontFamily: fonts.mono, fontSize: 11, color: c.brandSecondary, letterSpacing: 1.5, marginTop: spacing.md },
  ...
  runBtnText: { fontFamily: fonts.monoBold, fontSize: 14, color: c.onBrandSecondary, letterSpacing: 1 },
}));
